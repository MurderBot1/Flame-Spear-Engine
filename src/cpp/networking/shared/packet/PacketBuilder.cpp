...
template <typename T>
PacketBuilder<T>::PacketBuilder(int ID) : packet(ID), cursor(0) {}
...
void PacketBuilder<T>::writeBigEndian(U value) {
  static_assert(std::is_integral<U>::value,
                "writeBigEndian requires integral type");
...
template <typename T>
PacketBuilder<T> &
PacketBuilder<T>::withRawBytes(const std::vector<unsigned char> &var) {
