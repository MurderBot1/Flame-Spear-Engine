...
DOWNLOAD_LINK_WINDOWS: str = installutil.get_curl_command(
    f"https://github.com/ninja-build/ninja/releases/download/v{VERSION}/ninja-win.zip",
    DOWNLOAD_PATH_WINDOWS,
)
DOWNLOAD_LINK_LINUX: str = installutil.get_curl_command(
    f"https://github.com/ninja-build/ninja/releases/download/v{VERSION}/ninja-linux.zip",
    DOWNLOAD_PATH_LINUX,
)
DOWNLOAD_LINK_MAC: str = installutil.get_curl_command(
    f"https://github.com/ninja-build/ninja/releases/download/v{VERSION}/ninja-mac.zip",
    DOWNLOAD_PATH_MAC,
)

# Commands to install vulkan
WINDOWS_INSTALL_COMMANDS: list[str] = [
    f"move /Y {EXTRACT_PATH_WINDOWS / 'ninja.exe'} {INSTALL_PATH / 'ninja.exe'}"
]

LINUX_INSTALL_COMMANDS: list[str] = [
    f"mv {EXTRACT_PATH_LINUX / 'ninja'} {INSTALL_PATH / 'ninja'}"
]

MAC_INSTALL_COMMANDS: list[str] = [
    f"mv {EXTRACT_PATH_MAC / 'ninja'} {INSTALL_PATH / 'ninja'}"
]
...
# Downaloads the ninja executable installer and run it
def download_ninja() -> None:
